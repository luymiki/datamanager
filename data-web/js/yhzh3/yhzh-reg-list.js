    ...
    var reg = (function () {
        ...
        var _event = function () {
            $("#reginfo").on('click',function () {
                top.contabs.addMenuItem("/view/yhzh3/yhzh-reg.html",'导入银行账号信息');
            });

            $("#data-table").on('click','.jyjl',function () {
                top.contabs.addMenuItem("/view/yhzh3/jyjl/yhzh-jyjl-list.html?id="+$(this).attr("data-id"),'['+$(this).attr("data-name")+']交易记录列表');
            });
            ...
            $("#data-table").on('click','.detail',function () {
                top.contabs.addMenuItem("/view/yhzh3/yhzh-reg-detail.html?id="+$(this).attr("data-id"),'查看['+$(this).attr("data-name")+']开户信息');
            });
            $("#data-table").on('click','.analyze',function () {
                top.contabs.addMenuItem("/view/yhzh3/analyze/yhzh-analyze.html?id="+$(this).attr("data-id"),'银行流水信息分析');
            });
            ...
            swalMsg.msg({
                ...
                confirm:function (f) {
                    if(f) {
                        ...
                        $.ajax.proxy({
                            url: "/api/admin/yhzh3/khxx/delete",
                            type: "post",
                            dataType: "json",
