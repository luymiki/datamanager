package com.anluy.restful;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.serializer.SerializerFeature;
import com.anluy.restful.bean.Status;
import com.anluy.restful.utils.Configuration;
...
public class RestfulClient {
    private static Logger LOG = LoggerFactory.getLogger(RestfulClient.class);
    ...
    private static Map<String, RestClient> REST_CLIENT_CONTAINER = new HashMap<>();

    public RestfulClient(String hostPort) {
        this(hostPort, null, null, 5000, 60000, 60000);
    }

    public RestfulClient(String hostPort, String username, String password) {
        this(hostPort, username, password, 5000, 60000, 60000);
    }

    public RestfulClient(String hostPort, String username, String password, int connectTimeout, int socketTimeout, int maxRetryTimeoutMillis) {
        ...
        REST_CLIENT_CONTAINER.put(hostPort, restClient);
     ...
     * @return
     */
    public static RestClient getRestClient(String hostPort, String username, String password, int[] timeouts) {
     ...
     * @return
     */
    public Status batchSave(List<Map> recordDataList, String indexName, String typeName) {
        Status status = Status.ok();
        ...
        try {
            ...
            recordDataList.stream().forEach(map -> {
                ...
                if (map.containsKey("_parent")) {//save child document
            ...
            if (response.getStatusLine() != null) {
                int httpStatus = response.getStatusLine().getStatusCode();
                if (httpStatus == HttpStatus.SC_OK || httpStatus == HttpStatus.SC_CREATED) {//批量操作完成
                    Configuration configuration = createConfigFromResponseData(response);
                    if (LOG.isDebugEnabled()) {
                    ...
                    if (hasErrors) {//部分文档保存失败
                        ...
                        LOG.error(String.format("批量保存操作完成，总记录数：{%s}，成功：{%s}，失败：{%s}，response：{%s}", total, success, failure, errorItems.toString()));
                    }
                }
            } else {
                status = Status.error();
                status.setMessage("批量保存方法执行失败，请联系运维排查！");
            }
        } catch (Exception e) {
            LOG.error("batchSave方法执行异常！", e);
            status = Status.error();
            status.setMessage("批量保存方法执行失败，请联系运维排查！");
        }
     ...
     * @return
     */
    public Status batchSave(List<Map> recordDataList, String indexName, String typeName, boolean refresh) {
    ...
    public static void closeAll() {
        REST_CLIENT_CONTAINER.values().stream().forEach(restclient -> {
            try {
                restclient.close();
                LOG.info(String.format("关闭rest client客户端[%s]完成！", restclient));
            } catch (Exception e) {
