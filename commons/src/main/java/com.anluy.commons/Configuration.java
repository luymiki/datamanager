...
import com.anluy.commons.utils.StrUtil;
...
public class Configuration {
    ...
    public static Configuration from(File file) {
        try {
            ...
            throw new ConfigurationException(e, String.format("配置信息错误，您提供的配置文件[%s]不存在. 请检查您的配置文件.", file.getAbsolutePath()));
        } catch (IOException e) {
            throw new ConfigurationException(e, String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", file.getAbsolutePath(), e));
        }
    }
    ...
    public static Configuration from(InputStream is) {
        try {
            ...
            throw new ConfigurationException(e, String.format("请检查您的配置文件. 您提供的配置文件读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", e));
        }
    }
    ...
    public Configuration getConfiguration(final String path) {
        ...
        if (null == object) {
            ...
            return Configuration.from((JSONObject)object);
        }
    ...
    public Character getChar(final String path) {
        ...
        try {
            ...
            throw new ConfigurationException(e, String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Integer getInt(final String path) {
        ...
        try {
            ...
            throw new ConfigurationException(e, String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Double getDouble(final String path) {
        ...
        try {
            ...
            throw new ConfigurationException(e, String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型: %s. 请检查您的配置并作出修改.", path,
                    e.getMessage()));
        }
    }
    ...
    public Object remove(final String path) {
        ...
        if (null == result) {
            throw new ConfigurationException(String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误. 请联系DataX团队的同学.", path));
        }
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw new ConfigurationException(String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误, 请联系DataX开发团队!",
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw new ConfigurationException("Path不能为null，该异常代表系统编程错误, 请联系DataX开发团队 !");
        }
    ...
    Object setObjectRecursive(Object current, final List<String> paths,
        ...
        throw new ConfigurationException("该异常代表系统编程错误, 请联系DataX开发团队 !");
    }
    ...
    private static void checkJSON(final String json) {
        if (StringUtils.isBlank(json)) {
            throw new ConfigurationException("配置信息错误. 因为您提供的配置信息不是合法的JSON格式, JSON不能为空白. 请按照标准json格式提供配置信息. ");
        }
    }
    ...
    private Configuration(final String json) {
        try {
            ...
            throw new ConfigurationException(e, String.format("配置信息错误. 您提供的配置信息不是合法的JSON格式: %s . 请按照标准json格式提供配置信息. ", e.getMessage()));
        }
    }
    /**
