...
public class CSVReader implements Closeable {
    ...
    public CSVReader(char separator) {
        this(null,separator,DEFAULT_QUOTE_CHARACTER, DEFAULT_ESCAPE_CHARACTER);
    }
     ...
     * @throws IOException
     */
    public List<String> readAllLine(int tghs,int redLength) throws IOException {
        List<String> allElements = new ArrayList<String>();
        int index = 0;
        while (hasNext) {

            //跳行
            if (tghs > 0 && tghs > index){
                continue;
            }
            ...
            if (allElements.size() >= redLength && redLength > 0) {
                break;
            }
            index++;
        }
     ...
     * @throws IOException if bad things happen during the read
     */
    public List<String> parseLine(String nextLine) throws IOException {
        ...
        do {
            ...
            for (int i = 0; i < nextLine.length(); i++) {
                ...
                if (c == this.escape) {
                    ...
                    tokensOnThisLine.add(sb.toString().replace("\t",""));
