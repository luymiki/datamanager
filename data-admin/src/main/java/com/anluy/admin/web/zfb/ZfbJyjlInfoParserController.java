...
import com.anluy.admin.web.zfb.parser.ZfbTxParser;
 ...
 */
@RestController
@RequestMapping("/api/admin/zfb/jyjl")
@Api(value = "/api/admin/zfb/jyjl", description = "支付宝交易记录文件操作")
public class ZfbJyjlInfoParserController {
    private static final Logger LOGGER = LoggerFactory.getLogger(ZfbJyjlInfoParserController.class);
    @Resource
     ...
     * @return
     */
    @ApiOperation(value = "支付宝交易记录文件保存", response = Result.class)
    ...
    public Object save(HttpServletRequest request, @RequestBody  Attachment attachment) {
        try {
            ...
            ZfbJyjlParser parser = new ZfbJyjlParser(attachment);
            List<ZfbJyjlInfo> zfbJyjlInfoList = parser.parser(path);
            ...
            zfbJyjlInfoList.forEach(regInfo->{
                ...
                regInfo.setUserId(regInfo.getMjId());
            ...
            elasticsearchRestClient.batchSave(saveList,"zfbjyjlinfo");
...
//     * @return
//     */
//    @ApiOperation(value = "支付宝交易记录文件删除", response = Result.class)
