 ...
 */
@RestController
@RequestMapping("/api/admin/zfb/txinfo")
@Api(value = "/api/admin/zfb/txinfo", description = "支付宝提现记录文件操作")
public class ZfbTxInfoParserController {
    private static final Logger LOGGER = LoggerFactory.getLogger(ZfbTxInfoParserController.class);
    @Resource
     ...
     * @return
     */
    @ApiOperation(value = "支付宝提现记录文件保存", response = Result.class)
    ...
    public Object save(HttpServletRequest request, @RequestBody  Attachment attachment) {
        try {
            ...
            ZfbTxParser parser = new ZfbTxParser(attachment);
            List<ZfbTxInfo> zfbTxInfoList = parser.parser(path);
            List<Map> saveList = new ArrayList<>();
            zfbTxInfoList.forEach(regInfo->{
            ...
            elasticsearchRestClient.batchSave(saveList,"zfbtxinfo");
...
//     * @return
//     */
//    @ApiOperation(value = "支付宝提现记录文件删除", response = Result.class)
